 ...
 * @property {boolean} [options.extendQueries=false]
 *   If `true` the results from `find()` and `get()` will be passed through
 *   `this.extender.extend()` before being returned.
...
class LumDOM
{
   ...
   * @returns {boolean}
   * @alias module:@lumjs/dquery/util.isNodeType
   */
   ...
   *   See the `PARSE_TYPE` enum for a full list of valid values.
   * 
   ...
   * 
   *   What element is returned depends on the parsed content: 
   * 
   ...
   * - If there is *more than one*, then the `<body/>` itself will be returned.
   * 
   *   If `this.options.extendQueries` is `true` then the return value
   ...
   * 
   * If `this.options.extendQueries` is `true`, the result
  ...
  { 
    ...
    if (this.options.extendQueries && res)
    {
      this.extend(res);
    }
    return res;
